...
st.markdown("""
Aplikasi: Upload data, proses, atur parameter (Area, Anggaran, Batasan Cluster), lalu jalankan optimasi.
""")
...
if uploaded_file:
    ...
    if process_button:
        with st.spinner("Memproses data..."):
            # ... (Logika proses data dari langkah sebelumnya tetap sama) ...
...
if 'agg' in st.session_state:
    ...
    # Filter dataframe utama berdasarkan area yang dipilih
    ...
    if run_opt:
        ...
        with st.spinner("Menjalankan optimasi..."):
            ...
            # --- MODIFIKASI LOGIKA CLUSTER ---
            # Loop untuk menambahkan constraint HANYA jika nilainya lebih dari 0
            for cluster_name, max_pct in cluster_pct_inputs.items():
                if max_pct > 0:
                    members = agg_final[agg_final['Cluster'] == cluster_name]['ID Toko'].tolist()
                    # Kapasitas dihitung sebagai persentase maksimal dari N_max
                    cap = int(math.floor((max_pct / 100.0) * float(N_max)))
                    if members:
                        prob += pulp.lpSum([x_vars[sid] for sid in members]) <= cap
                        st.write(f"ℹ️ Batasan diterapkan: Cluster **{cluster_name}** tidak boleh lebih dari **{cap} toko**.")
            # --- AKHIR MODIFIKASI LOGIKA CLUSTER ---
